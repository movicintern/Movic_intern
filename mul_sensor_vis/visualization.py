...
class Visualization:
    ...
    def save_png(self, instance: Setup, data1, data2):
        ...
        axes[0].imshow(dB1, extent=[0, 3, 0, int(384000 / 1000 / 2)], cmap='jet', aspect='auto')
        axes[0].set(title=f'{instance.sensor_nm[0]} ({data_nm1}.wav)', ylabel='Frequency (Hz)', xlabel='Time (sec)')
        
        dB2 = np.flip(dB2, axis=0)
        axes[1].imshow(dB2, extent=[0, 3, 0, int(384000 / 1000 / 2)], cmap='jet', aspect='auto')
        axes[1].set(title=f'{instance.sensor_nm[1]} ({data_nm2}.wav)', ylabel='Frequency (Hz)', xlabel='Time (sec)')

        save_path = os.path.join(instance.save_directory, f'multi_sensor/3_motor-10kt/Date_{instance.start_point}~{instance.end_point}', f'{instance.sensor_nm[0]}_{instance.sensor_nm[1]}/{data_nm1}_{data_nm2}.png')
